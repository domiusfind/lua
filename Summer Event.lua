...
local sellThread = nil -- thread de execução
...
v16.Main:AddToggle("ToggleAutoSellSummer", {
    ...
    Description = "sell all summer fruits",
    Default = false,
    Callback = function(state)
                ...
                while autoSellSummer do
                    ...
                    for _, item in pairs(backpack:GetChildren()) do
                        if item:IsA("Tool") then
                            ...
                            local fruitBaseName = name:match("^(.-)%s*%[.-kg%]$")

                            if fruitBaseName and fruitsToSell[fruitBaseName] then
                                if humanoid then
                                ...
                                -- Enviar evento
                                game:GetService("ReplicatedStorage").GameEvents.SummerHarvestRemoteEvent:FireServer("SubmitHeldPlant")
                                task.wait(0.2)
                            end
                        end
                    end
